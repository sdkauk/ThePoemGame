...
export const userService = {
  ...
  joinGroup: async (inviteCode: string): Promise<boolean> => {
    const response = await authFetch(
      `${API_BASE_URL}/api/users/group/join?inviteCode=${inviteCode}`,
      {
        method: "POST",
      }
    );
  ...
  leaveGroup: async (groupId: string): Promise<boolean> => {
    const response = await authFetch(
      `${API_BASE_URL}/api/users/group/leave/?groupId=${groupId}`,
      {
        method: "POST",
      }
    );
  ...
  joinGameByInviteCode: async (inviteCode: string): Promise<boolean> => {
    const response = await authFetch(
      `${API_BASE_URL}/api/users/game/join/invite/${inviteCode}`,
      {
        method: "POST",
      }
    );
  ...
  joinGameById: async (gameId: string): Promise<boolean> => {
    const response = await authFetch(
      `${API_BASE_URL}/api/users/game/join/id/${gameId}`,
      {
        method: "POST",
      }
    );
  ...
  leaveGame: async (gameId: string): Promise<boolean> => {
    const response = await authFetch(
      `${API_BASE_URL}/api/users/game/leave/?gameId=${gameId}`,
      {
        method: "POST",
      }
    );
